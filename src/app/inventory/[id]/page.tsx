...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const description =
    ...
    `${title} for sale at Signature Motors. ${formatPrice(vehicle.price, vehicle.currency)}. ${vehicle.mileage.toLocaleString()} km.`;
...
export default async function VehicleDetailPage({ params }: PageProps) {
  ...
  const whatsappMessage = encodeURIComponent(
    `Hello Signature Motors, I'm interested in the ${vehicle.year} ${vehicle.make} ${vehicle.model} listed on your website (ID: ${vehicle.id}). Is it still available?`
  );
  const whatsappUrl = `https://wa.me/2348000000000?text=${whatsappMessage}`;

  const shareUrl = `https://signaturemotors.com/inventory/${vehicle.id}`; // replace with real domain when live
            ...
            <div className="space-y-3">
              {!isSold ? (
                <a
                  ...
                  className="flex w-full items-center justify-center gap-2 rounded-full bg-[#25D366] px-6 py-3.5 text-base font-semibold text-white transition hover:bg-[#20bd5a] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold"
                >
              ...
              <div className="flex gap-3">
                <a
                  ...
                  className="flex flex-1 items-center justify-center gap-2 rounded-full border border-border px-4 py-2.5 text-sm font-medium transition hover:border-gold hover:text-gold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold"
                >
